...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let questions =
        isEditMode && loadedQuestions.length > 0
        ...
        questions.forEach((q, index) => {
            ...
            <div class="question-meta-row">
                ...
                <select class="question-points" data-q="${index}">
                    <option value="10" ${
                        q.points === 10 ? "selected" : ""
                    }>10 pts</option>
                    <option value="20" ${
                        q.points === 20 ? "selected" : ""
                    }>20 pts</option>
                    <option value="30" ${
                        q.points === 30 ? "selected" : ""
                    }>30 pts</option>
                    <option value="50" ${
                        q.points === 50 ? "selected" : ""
                    }>50 pts</option>
                </select>
            </div>
            ...
            <div class="answer-list">
                ${q.answers
                    ...
                    <div class="answer-item">
                        <i class='bx ${
                            q.correctAnswers[i]
                                ? "bx-checkbox-checked checked"
                                : "bx-checkbox"
                        }' data-q="${index}" data-a="${i}"></i>
    ...
    document.addEventListener("click", function (e) {
        if (e.target.closest(".delete-question")) {
            const index = parseInt(
                e.target.closest(".delete-question").dataset.index
            );
        ...
        if (
            ...
            e.target.classList.contains("bx-checkbox-checked")
        ) {
            const qIndex = parseInt(e.target.dataset.q);
            const aIndex = parseInt(e.target.dataset.a);
            questions[qIndex].correctAnswers[aIndex] =
                !questions[qIndex].correctAnswers[aIndex];
            renderAllQuestions();
        }
    });
