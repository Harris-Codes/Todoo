...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const assignmentTableBody = document.querySelector(
        ".assignment-table tbody"
    );
    if (assignmentTableBody) {
        const rows = assignmentTableBody.querySelectorAll("tr");
        if (rows.length > 4) {
            assignmentTableBody.parentElement.classList.add(
        ...
        document.querySelectorAll(".clickable-row").forEach((row) => {
            row.addEventListener("click", function () {
                ...
                const submissionWrapper =
                    document.getElementById("submissionWrapper");
            ...
            <div class="graded-card">
              ...
              <div class="graded-info">
                  ...
                  <p>You have received a grade of <strong>${
                      grade ?? "-"
                  }</strong> for this assignment.</p>
        ...
        fetch(`/classroom/${classroomId}/folders/list`)
            ...
            .then((data) => {
              ...
              `;
                        ...
                        document
                            .querySelectorAll(".clickable-folder-row")
                            .forEach((row) => {
                                row.addEventListener("click", function () {
        ...
        fetch(`/quiz/${quizId}/leaderboard`)
            ...
            .then((data) => {
                ...
                data.slice(3).forEach((entry, index) => {
                    ...
                    row.innerHTML = `<span>${index + 4}. ${
                        entry.user.name
                    }</span><span>${entry.score} pts</span>`;
    ...
    document.querySelectorAll(".clickable-badge-row").forEach((row) => {
        row.addEventListener("click", () => {
            ...
            document.getElementById("badgeInfoName").textContent =
                row.dataset.name;
            ...
            if (type === "submission_count") {
                ...
                conditionText =
                    "Earn this badge by completing the required condition.";
            }

            document.getElementById("badgeInfoCondition").textContent =
                conditionText;
